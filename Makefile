...
# =====================================================================
.PHONY: bootstrap prereqs images-push upload-sample

bootstrap: prereqs gcp-setup tf-bootstrap images-push tf-apply upload-sample ingestion-run bq-vector-index
	@echo "=== bootstrap 完了 ==="
...
images-push: ingestion-push qa-api-push eval-pipeline-push
...
gcp-setup:
	@scripts/setup/gcp_setup.sh $(PROJECT_ID) $(REGION) $(TF_SA) $(COMPUTE_SA)
...
tf-import-registry: tf-init
	@scripts/setup/tf_import_registry.sh $(TF_DIR) $(PROJECT_ID) $(REGION) $(REPO)
...
deploy-all: bootstrap

destroy-all: tf-destroy
	@echo "=== 全リソース削除完了 ==="
